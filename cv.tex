...
\runsubsectionlower{Cloud:}\descript{Microsoft Azure} & \runsubsectionlower{Programming:}\descript{PHP $\cdot$ C# $\cdot$ Java} \\[12pt]
\runsubsectionlower{IaC:}\descript{Terraform $\cdot$ HCP} & \runsubsectionlower{UI:}\descript{Joomla $\cdot$ WordPress $\cdot$ Angular $\cdot$ React $\cdot$ Asp.Net} \\[12pt]
\runsubsectionlower{CI/CD:}\descript{Azure DevOps $\cdot$ GitHub Actions} & \runsubsectionlower{Containers:}\descript{Docker $\cdot$ K8S} \\[12pt]
\runsubsectionlower{VCS:}\descript{GitHub $\cdot$ Azure Repos $\cdot$ Bitbucket} & \runsubsectionlower{Templating:}\descript{Jinja2 $\cdot$ Nunjucks $\cdot$ FreeMarker} \\[12pt]
\runsubsectionlower{Scripting:}\descript{Bash $\cdot$ PowerShell $\cdot$ Python} & \runsubsectionlower{CSS:}\descript{sass $\cdot$ scss $\cdot$ less} \\[12pt]
\runsubsectionlower{CaC:}\descript{Ansible (roles \& playbooks)} & \runsubsectionlower{Agile:}\descript{SAFe $\cdot$ Scrum $\cdot$ Kanban} \\[12pt]
\runsubsectionlower{Artifacts:}\descript{Artifactory $\cdot$ ACR $\cdot$ Azure Artifacts} & \runsubsectionlower{Collaboration:}\descript{Jira $\cdot$ Confluence $\cdot$ Teams $\cdot$ Slack}
\end{tabular}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     Education
